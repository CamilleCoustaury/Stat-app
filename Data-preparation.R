...
describe(index)
# we have 12,099 participants in the first wave.
...
#participant in the data set, drawing on the wave 1 core file. We should keep
#in mind that the sclddr variable only comes from wave 1, and so we will
...
# including: In wave 1, self-rated health has a lot of missing information, and
# in wave 3 it was not measured at all.

#A look at the data set from wave 1 shows that self-rated health has been
...
# I assume that wave 1 included a so-called split ballot experiment where one
...
# that analyze self-rated health using ELSA data from 10-15 years ago to get a
